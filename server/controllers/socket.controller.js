...
export const socketCtrl = (io , getDocThroughSocket) => {
  io.on('connection', (socket) => {
    const userId = socket.id;
    handleConnection(socket, io, userId, getDocThroughSocket);
  });
};

const handleConnection = async (socket, io, userId, getDocThroughSocket) => {
  ...
  socket.on('save-doc', async (data, callback) => {
    try {
      ...
      if (!data.data) return;
      await Document.findByIdAndUpdate(data?.docId?.toString(), { content: data?.data });
